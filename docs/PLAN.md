...
| サブタスク | 状態 |
|-----------|------|
| `[x]` タスク定義の TypeScript 型定義 |
| `[x]` YAML/JSON パーサー実装 |
| `[x]` JSON Schema によるバリデーション |
| `[x]` バリデーションエラーメッセージ |
| `[x]` タスクIDの一意性チェック |
| `[x]` デフォルト値の適用ロジック |
...
**完了条件**:
- [x] サンプル tasks.yaml が正常にパースできる
- [x] 不正な YAML に対してわかりやすいエラーが出る
...
| サブタスク | 状態 |
|-----------|------|
| `[x]` Git 操作ラッパー（simple-git使用） |
| `[x]` ブランチ作成・削除 |
| `[x]` git worktree 作成・削除 |
| `[x]` コミット・diff取得 |
| `[x]` rebase 実行 |
| `[x]` コンフリクト検出 |
| `[x]` ブランチ命名規則の適用 |
...
**完了条件**:
- [x] costTier + complexityHint からモデルが選択できる
- [x] コスト計算が正しく動作する
- [x] 予算超過時に警告が出る
